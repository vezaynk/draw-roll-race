...
import { BASE, launch, newPlayer, drawWheel, drawSpokes, text, waitForText } from './helpers.mjs';
...
test('spikes shatter legs, and spokes climb out of the pit', async () => {
  ...
  await drawWheel(p);
  await p.waitForFunction(() => DRRGame.state.limbs.leg.length === 0 && DRRGame.state.time > 0, null, { timeout: 20000 });
  assert.match(await p.textContent('#toast'), /Legs shattered/);
  await drawSpokes(p);
  await p.waitForFunction(() => DRRGame.state.player.x > DRRGame.state.course.sections[0].to, null, { timeout: 20000 });
  assert.deepEqual(p.errors, []);
  await p.context().close();
});
