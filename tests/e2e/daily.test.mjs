...
import { BASE, launch, newPlayer, drawWheel, text, waitForText } from './helpers.mjs';
...
test('opponents from Options race, and your best run comes back as a ghost', async () => {
  ...
  assert.equal(await p.evaluate(() => !document.querySelector('#progress .dot.ghost').hidden), true, 'ghost dot visible');
  assert.deepEqual(p.errors, []);
  await p.context().close();
});

test('daily leaderboard accepts real runs and refuses impossible ones', async () => {
  const info = await (await fetch(BASE + 'api/daily')).json();
  ...
  await p.goto(BASE);
  const course = await p.evaluate(stage => { const c = DRR.buildCourse(stage); return { startX: c.startX, finishX: c.finishX }; }, info.stage);
  const speed = 250, time = (course.finishX - course.startX) / speed;
  const trace = [];
  for (let t = 0; t <= time; t += 0.2) trace.push([+t.toFixed(2), course.startX + speed * t]);
  trace.push([+time.toFixed(2), course.finishX]);
  const player = 'e2e-player-' + Date.now().toString(36).padStart(8, '0');
  const post = body => fetch(BASE + 'api/daily', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(body) });

  const ok = await post({ day: info.day, player, name: 'Hana', time: +time.toFixed(2), trace });
  assert.equal(ok.status, 200, await ok.text());
  const board = await (await fetch(BASE + 'api/daily?player=' + player)).json();
  ...
  const fast = await post({ day: info.day, player, name: 'Hana', time: 0.2, trace });
  assert.equal(fast.status, 422);
  const jump = await post({ day: info.day, player, name: 'Hana', time: 5, trace: [[0, course.startX], [0.5, course.startX + 50], [5, course.finishX]] });
  assert.equal(jump.status, 422);
  assert.match((await jump.json()).error, /gaps/);
  const teleport = [[0, course.startX]];
  for (let t = 0.5; t < 5; t += 0.5) teleport.push([t, course.startX + 20 * t]);
  teleport.push([5, course.finishX]);
  const zap = await post({ day: info.day, player, name: 'Hana', time: 5, trace: teleport });
  assert.equal(zap.status, 422);
  assert.match((await zap.json()).error, /faster than any runner/);
