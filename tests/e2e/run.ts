// Starts the Worker locally (with test courses allowed), waits for it, runs the browser tests,
// then stops it.
// Usage: npm run test:e2e   (or npx tsx tests/e2e/run.ts [test files...])
...
try {
  ...
  const files = process.argv.slice(2).length
    ...
    : readdirSync(here).filter((f) => f.endsWith('.test.ts')).map((f) => path.join(here, f));
  code = await new Promise<number>((resolve) => {
    const t = spawn(process.execPath, ['--import', 'tsx', '--test', '--test-concurrency=1', ...files], {
      stdio: 'inherit',
      env: {
        ...process.env, E2E_BASE: BASE, NO_PROXY: '127.0.0.1,localhost', no_proxy: '127.0.0.1,localhost',
      },
    });
